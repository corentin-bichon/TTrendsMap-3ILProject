...
def getHtmlTopTweet(country_name):
    ...
    # Titre de la popup
    tweethtml = "<h2 style='text-align:center; min-width:105px; margin-bottom:2px; color:#3388FF'> Trends </h2>" + "<h4 style='text-align:center; margin-top:0px'>" + getcountriesbyid(id_country) + "</h4>"
    # On ouvre la liste
    ...
    for tweets_country in top_tweet_list['Top tweets']:
        for tweets in tweets_country:
            for tweetlocation in tweets['locations']:
                if tweetlocation['woeid'] == id_country:
                    for singletweet in tweets['trends']:
                        tweethtml += "<li> <a href='" + singletweet['url'] + "' target='blank'>" + singletweet['name'] + "</a> </li>"
                        tweet_count += 1
                        if tweet_count == 10:
                            break
            break
